...
```
npm install @bundy-lmw/hive-core → new Agent() → agent.dispatch()
```
...
安装 `@bundy-lmw/hive-core`，设置环境变量或传入配置，调用 `agent.dispatch()` 即可。也提供了开箱即用的 HTTP 服务 `@bundy-lmw/hive-server`。
...
实现 Plugin 接口，打包为独立 npm 包。参考 `@bundy-lmw/hive-plugin-feishu` 的实现。
