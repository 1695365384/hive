...
The system SHALL maintain a pricing table for supported models, stored in SQLite `models` table (`cost_input`, `cost_output`, `cost_cache_read` columns).

#### Scenario: Pricing table includes core models
- **WHEN** system initializes and syncs from models.dev
- **THEN** SQLite `models` table SHALL include cost data for all models that have pricing in models.dev
...
#### Scenario: ModelSpec query uses SQLite
- **WHEN** `fetchModelSpec()` is called and `SqlitePersistence` is available
- **THEN** system SHALL query SQLite `models` table directly for cost and context window data, without loading full cache
