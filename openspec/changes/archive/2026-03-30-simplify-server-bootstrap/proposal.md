...
`apps/server/bootstrap.ts` 目前 509 行，承担了太多职责：插件加载、数据库创建、定时任务引擎初始化、HeartbeatScheduler、心跳订阅、Session→Channel 路由映射、消息总线订阅编排。这些本该是 SDK 的内置逻辑，却全扔给了应用层。同时 `@bundy-lmw/hive-orchestrator` 包包含废弃的 `PluginHost` 和 `Scheduler`，只在 `apps/server` 里用到了一个 `MessageBus`，其余都是死代码。

## What Changes

- 将 `MessageBus` 从 `@bundy-lmw/hive-orchestrator` 迁移到 `@bundy-lmw/hive-core/src/bus/`
- 删除 `@bundy-lmw/hive-orchestrator` 包的剩余内容（PluginHost、Scheduler、AgentPool）
...
- `message-bus`: 将 `MessageBus` 从 `@bundy-lmw/hive-orchestrator` 迁移到 `@bundy-lmw/hive-core/src/bus/`，保留所有现有功能（pub/sub、request/response、wildcard、middleware）。`@bundy-lmw/hive-orchestrator` 包删除。
...
- `@bundy-lmw/hive-orchestrator` 包删除，apps/server 和 apps/cli 的 import 路径从 `@bundy-lmw/hive-orchestrator` 改为 `@bundy-lmw/hive-core`
