...
本 skill 最常见失败不是“能不能生成”，而是 **文案像模型在写稿**、以及 **字塞爆模板**。生成时强制遵守：

- **人话、短句、具体**：写事实、数字、动作、结论；禁止「赋能 / 助力 / 开启新篇章 / 深度洞察 / 全面赋能 / 颠覆式创新 / 让世界更美好」等空话。
- **不要中英口号混排**，不要堆 emoji，不要排比抒情。
- **严守 `fillPlan` / `copyBudgets.maxChars`**：标题短；`display` / `metric` 只写数字或短词；超长先换 layout，禁止硬塞导致溢出/遮挡。
- **一页一个信息角色**：不要把多段演讲稿塞进单页。
- **`copyKeys` 全覆写**：漏填会露出模板默认文案，视为交付失败。
- 渲染后必须过 `validate:goal-spec`、`validate:swiss`、`validate:goal-copy`；有浏览器时目检溢出/裁切/重叠。
