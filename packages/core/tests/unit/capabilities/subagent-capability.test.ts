...
describe('SubAgentCapability', () => {
  ...
  describe('扩展 Agent 方法', () => {
    describe('reviewCode()', () => {
      it('should review code', async () => {
        const result = await capability.reviewCode('src/auth.ts');

        expect(result).toBe('Agent completed successfully');
        expect(context.runner.execute).toHaveBeenCalledWith(
          'code-reviewer',
          expect.stringContaining('Review the code: src/auth.ts')
        );
      });
    });
    ...
    describe('generateTests()', () => {
      it('should generate tests', async () => {
        const result = await capability.generateTests('src/utils.ts');

        expect(result).toBe('Agent completed successfully');
        expect(context.runner.execute).toHaveBeenCalledWith(
          'test-engineer',
          expect.stringContaining('Generate tests for: src/utils.ts')
        );
      });
    });

    describe('writeDocs()', () => {
      it('should write documentation', async () => {
        const result = await capability.writeDocs('src/api.ts');

        expect(result).toBe('Agent completed successfully');
        expect(context.runner.execute).toHaveBeenCalledWith(
          'doc-writer',
          expect.stringContaining('Write documentation for: src/api.ts')
        );
      });
    });
    ...
    describe('debug()', () => {
      it('should debug issue', async () => {
        const result = await capability.debug('Null pointer in auth');

        expect(result).toBe('Agent completed successfully');
        expect(context.runner.execute).toHaveBeenCalledWith(
          'debugger',
          expect.stringContaining('Debug this issue: Null pointer in auth')
        );
      });
    });
    ...
    describe('refactor()', () => {
      it('should refactor code', async () => {
        const result = await capability.refactor('src/legacy.ts');

        expect(result).toBe('Agent completed successfully');
        expect(context.runner.execute).toHaveBeenCalledWith(
          'refactorer',
          expect.stringContaining('Refactor this code: src/legacy.ts')
        );
      });
    });
    ...
    describe('securityAudit()', () => {
      it('should perform security audit', async () => {
        const result = await capability.securityAudit('src/auth.ts');

        expect(result).toBe('Agent completed successfully');
        expect(context.runner.execute).toHaveBeenCalledWith(
          'security-auditor',
          expect.stringContaining('Audit security of: src/auth.ts')
        );
      });
    });
  });
