...
describe('Agent.runHeartbeatOnce()', () => {
  ...
  it('回复 HEARTBEAT_OK 时 isOk 应为 true', async () => {
    vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: 'HEARTBEAT_OK', success: true, duration: 0, tools: [] });
  ...
  it('回复 HEARTBEAT_OK: extra info 时 isOk 应为 true', async () => {
    vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: 'HEARTBEAT_OK: All systems nominal', success: true, duration: 0, tools: [] });
  ...
  it('回复非 HEARTBEAT_OK 时 hasAlert 应为 true', async () => {
    vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: 'WARNING: Build is failing on CI', success: true, duration: 0, tools: [] });
  ...
  it('回复包含 alert 内容时应保留完整内容', async () => {
    ...
    vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: alertText, success: true, duration: 0, tools: [] });
  ...
  it('使用默认 prompt 时应包含 HEARTBEAT.md 指令', async () => {
    const dispatchSpy = vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: 'HEARTBEAT_OK', success: true, duration: 0, tools: [] });
  ...
  it('使用自定义 prompt 时应覆盖默认值', async () => {
    ...
    expect(dispatchSpy).toHaveBeenCalledWith(customPrompt, expect.any(Object));
  });
  ...
  it('onResult 回调应包含 alert 内容', async () => {
    ...
    vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: alertContent, success: true, duration: 0, tools: [] });
  ...
  it('应支持指定 model', async () => {
    ...
    expect(dispatchSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ modelId: 'claude-haiku-4-5-20251001' }));
  });

  it('回复前后有空白时仍能正确解析 HEARTBEAT_OK', async () => {
    vi.spyOn(agent, 'dispatch').mockResolvedValue({ text: '  \n  HEARTBEAT_OK  \n  ', success: true, duration: 0, tools: [] });
